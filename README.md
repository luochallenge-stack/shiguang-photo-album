# 伞兵训练营的时光集

一个需要账号登录、受控管理的在线影像集：支持站内注册登录、成员称号、访问/上传/编辑/删除/文件夹/赋予称号六项独立权限、按身份控制文件夹可见范围、访问与修改日志、批量照片/视频上传、批量移动、7 天回收站、上传进度、网格/列表浏览、在线播放、重命名、复制文件夹链接和下载。

面向后续开发者和 AI 编码代理的架构、核心文件、权限边界与发布说明见 [`develop.md`](./develop.md)。

照片支持 JPEG、PNG、WebP、GIF、HEIC、HEIF，单张最大 50 MB；视频支持 MP4、MOV、M4V、WebM、MPEG，单个最大 500 MB。浏览器使用服务端签发的单文件临时凭证直传 CloudBase 云存储，避免大文件经过云托管请求体。

线上地址：<https://paratrooper-battalion-d1b3b82e83-1313194650.ap-shanghai.app.tcloudbase.com/>

微信小程序原生客户端位于 [`miniprogram`](./miniprogram)，AppID 为 `wx5c6f75fd860fb659`。它与网页端共用账号、权限、称号、日志、数据库和云存储；Logo 旁的三横菜单用于打开文件目录，各项按钮按当前账号的独立权限显示。阿里山清茶会额外看到“相册 / 用户权限”Tab 和独立权限页面，其他用户看不到该入口；成员称号在双端使用金色流光效果展示。

旧的云托管域名会自动跳转到上述官方 Web 安全域名，并保留文件夹和上传链接参数。

CloudBase 环境：`paratrooper-battalion-d1b3b82e83`，云托管服务：`sanbing`。

## 腾讯云架构

- CloudBase 云存储保存照片与视频原文件，并通过临时 CDN 链接在线查看和下载。
- CloudBase 文档数据库保存用户、审计日志、文件夹、影像索引和上传链接权限。
- CloudBase 服务端 API Key 只存在服务端环境变量中，不会发送到浏览器。
- 所有访问者必须先登录。注册成功后默认拥有访问权限；上传、编辑、删除和文件夹管理互相独立，均由超级管理员按人授权。
- 账号 `alishan-tea`（阿里山清茶）是唯一超级管理员，默认称号为“伞兵指挥官”并始终拥有全部权限。只有该账号能看到完整权限管理；获得“赋予称号”权限的人只能进入称号管理，看不到其他人的权限设置。
- 文件夹可设为“所有人可见”“管理者可见”或“某些人可见”。拥有文件夹管理权限的创建者和项目超级管理员可以修改可见范围。
- “全部影像”、文件夹列表、详情、下载、回收站和修改接口都使用同一套身份鉴权；无权限用户不会看到文件夹或其中任何内容。
- CloudBase Run 承载完整 Next.js 服务，默认域名可在中国大陆直接访问。

## 本地运行

```bash
npm install
cp .env.example .env.local
npm run dev
```

需要配置：

- `CLOUDBASE_ENV_ID`
- `CLOUDBASE_APIKEY`
- `ALBUM_ADMIN_KEY`
- `ALBUM_SESSION_SECRET`（建议使用独立的高强度随机值）
- `ALBUM_PUBLIC_ORIGIN`（线上相册域名）

注册用户的密码只保存为带随机盐的 PBKDF2-SHA256 摘要。新账号默认只有访问权限；阿里山清茶可在“权限与日志”中逐项授权或停用账号。`ALBUM_ADMIN_KEY` 是管理员应急入口，但不等同于项目超级管理员。

## 腾讯云部署

```bash
tcb login
tcb cloudrun deploy --port 3000
```

## 验证

```bash
npm run build
npm test
```

小程序可以直接将 `miniprogram` 目录导入微信开发者工具。服务器合法域名和发布检查见 [`miniprogram/README.md`](./miniprogram/README.md)。
