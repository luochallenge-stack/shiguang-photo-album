...
微信小程序原生客户端位于 [`miniprogram`](./miniprogram)，AppID 为 `wx5c6f75fd860fb659`。它与网页端共用账号、权限、日志、数据库和云存储；普通成员可浏览、解锁文件夹、预览图片并播放视频，管理员还可在小程序内向指定文件夹批量上传图片。其余管理操作继续在网页端完成。
...
- 所有访问者必须先登录。注册成功后自动成为普通成员并可立即浏览；管理员可创建文件夹、上传、重命名或永久删除影像，每个文件夹可生成独立上传链接。
