...
一个需要账号登录、受控管理的在线影像集：支持站内注册登录、用户角色、按身份控制文件夹可见范围、访问与修改日志、批量照片/视频上传、批量移动、7 天回收站、上传进度、网格/列表浏览、在线播放、重命名、复制文件夹链接和下载。
...
微信小程序原生客户端位于 [`miniprogram`](./miniprogram)，AppID 为 `wx5c6f75fd860fb659`。它与网页端共用账号、权限、日志、数据库和云存储；Logo 旁的三横菜单用于打开文件目录，管理员可新建、重命名、排序并设置文件夹可见范围，重命名单项文件，并向指定文件夹批量上传照片和带首帧封面的视频。用户授权在网页端完成，完整审计日志仅项目超级管理员可查看。
...
- 管理员可以管理成员角色与状态；完整访问、预览、下载和修改日志只对项目超级管理员开放。客户端 IP 只保存带密钥的不可逆摘要。
- 文件夹可设为“所有人可见”“管理员可见”或“某些人可见”。创建者和项目超级管理员可以修改可见范围。
