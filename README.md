...
微信小程序原生客户端位于 [`miniprogram`](./miniprogram)，AppID 为 `wx5c6f75fd860fb659`。它与网页端共用账号、权限、日志、数据库和云存储；Logo 旁的三横菜单用于打开文件目录，管理员可新建、重命名、排序和加密文件夹，重命名单项文件，并向指定文件夹批量上传照片和带首帧封面的视频。用户授权和日志查看继续在网页端完成。
