...
一个需要账号登录、受控管理的在线影像集：支持微信/QQ 登录、用户角色、访问与修改日志、虚拟文件夹、文件夹密码、批量照片/视频上传、上传进度、网格/列表浏览、在线播放、重命名、删除、复制文件夹链接和下载。
...
- 所有访问者必须先登录。普通成员只能浏览；管理员可创建文件夹、重命名或永久删除影像，每个文件夹可生成独立上传链接。
...
未配置微信或 QQ 凭证时，对应按钮会显示为等待配置，管理员仍可使用 `ALBUM_ADMIN_KEY` 登录。

第三方平台回调地址：

```text
https://你的域名/api/auth/callback/wechat
https://你的域名/api/auth/callback/qq
```
