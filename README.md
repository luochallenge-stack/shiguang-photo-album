...
一个公开浏览、受控管理的在线相册：支持虚拟文件夹、文件夹密码、批量图片上传、上传进度、网格/列表浏览、原图预览、重命名、删除、复制文件夹链接和下载。
...
- 普通访问者只能浏览；管理口令可创建文件夹、重命名或永久删除照片，每个文件夹可生成独立上传链接。
- 管理员可以为任意文件夹设置、更换或移除密码。密码使用带随机盐的 PBKDF2-SHA256 保存，不存储明文。
- 加密文件夹不会出现在“全部照片”的内容中；进入该文件夹并正确输入密码后，浏览器获得 12 小时的 HttpOnly 访问凭证。
