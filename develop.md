...
│   ├── pages/permissions/       超级管理员专属用户权限页面
...
- Web 大图预览支持左右按钮和 `Esc` 退出；小程序图片查看页使用 `swiper` 全屏左右滑动切换，不展示底部图片信息，再次点击放大图片退出，并开启图片长按菜单用于保存或转发。视频在列表、预览和小程序播放器中都按原始比例 `contain` 缩放。
...
- 新上传视频使用微信选择器返回的首帧缩略图作为列表封面，没有封面的历史视频继续显示占位状态；列表只渲染封面图，不要直接挂 `<video>`，否则多视频页面会同时抢元数据导致卡顿。
- 目录和全部影像只渲染服务端返回的可见文件夹与媒体，不在客户端保留隐藏目录占位。
- 页面按六项权限分别显示上传、编辑、删除、回收站和目录管理入口。
- “相册 / 用户权限”Tab 和 `pages/permissions/` 只对阿里山清茶显示；独立权限页先通过 `/api/auth/session` 复核身份，再读取完整用户权限。
- 被授予 `assignTitles` 的用户只看到相册页内的“称号管理”，不会收到权限字段或看到权限 Tab。
- 双端页头称号使用轻量金色流光效果；Web 遵循 `prefers-reduced-motion` 关闭动画。
...
- 小程序提供超级管理员专属用户权限 Tab 和独立页面，并按六项权限开放新建/重命名/排序/设置文件夹可见范围、单项文件重命名、批量移动、回收站和带首帧封面的照片/视频上传；完整日志查看仍在网页管理端完成。
