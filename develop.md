...
├── miniprogram/                 原生微信小程序
│   ├── pages/login/             注册和登录
│   ├── pages/library/           文件夹、分页浏览、解锁和管理员图片上传
...
1. `wx.chooseMedia` 最多选择 9 张图片。
2. 大于 8 MB 的图片使用 `wx.compressImage`，质量参数为 86。
3. `wx.uploadFile` 以 multipart 请求调用 `POST /api/photos`。
4. 服务端读取文件、校验管理员权限、上传云存储并写入数据库。

该链路会让文件内容经过 CloudBase Run 并在服务端转为 Buffer，因此不应直接用于小程序大视频。要增加小程序视频上传，应优先实现小程序可用的云存储直传，而不是放宽 multipart 请求限制。
...
| `GET /api/library` | 登录 | 文件夹和可见媒体列表；小程序支持 `limit/offset` |
...
- `uploadImage` 封装 `wx.uploadFile` 和上传进度。
